...
function Save-InstallLog {
    if (-not $script:logPath) {
        $script:logPath = Join-Path $PSScriptRoot ("_install-{0:yyyyMMdd-HHmmss}.log" -f $startedAt)
    }
...
function Invoke-WslWithTimeout {
    ...
    if (-not $Description) {
        $Description = "wsl " + ($Arguments -join ' ')
    }
    ...
    try {
        $job = Start-Job -ScriptBlock {
            param($WslPath, $WslArgs)

            try {
                $raw = & $WslPath @WslArgs 2>&1
                [pscustomobject]@{
                    ExitCode = $LASTEXITCODE
...
function Get-DockerDesktopExePath {
    ...
    if (Test-Path -LiteralPath $dockerRoot) {
        $found = Get-ChildItem -LiteralPath $dockerRoot -Recurse -Force -File -Filter 'Docker Desktop.exe' -ErrorAction SilentlyContinue |
...
function Wait-DockerDesktopExePath {
    param([int]$TimeoutSeconds = 180)

    $deadline = (Get-Date).AddSeconds($TimeoutSeconds)
    do {
        $path = Get-DockerDesktopExePath
        if ($path) { return $path }
...
function Get-DockerInstallerPath {
    ...
    $installer = Get-ChildItem -Path $DownloadDirectory -Recurse -File -ErrorAction SilentlyContinue |
    ...
    if (-not $installer) {
        throw "Docker Desktop installer was not found in $DownloadDirectory"
    }
