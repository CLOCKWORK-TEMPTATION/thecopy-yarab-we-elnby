...
مقروءة ومثبتة من `AGENTS.md`و `.repo-agent/OPERATING-CONTRACT.md`و `CLAUDE.md`. لم تُعدَّل أي ملفات فحص أو contract أو test config أو tsconfig أو eslint config.

### ما تم

- قياس baseline حقيقي:
  - backend = 1,374 errors
  - web = 1,449 errors + 36 warnings
- backend autofix شامل (2 تشغيلتين). تأثير محدود (3-4 ملفات معدَّلة)
- web autofix شامل بـ `--fix-type problem,suggestion,layout`→ 19 ملف معدَّل، -205 errors
- narrowing يدوي backend:
  - `services/cineai.service.ts` — استبدل `Record<string, any>`بـ zod schemas `ShotValidationResult`و `ColorPaletteResult`
    - إزالة eslint-disable
  - `services/agents/shared/enhancedCritiqueHelpers.ts` — type guard `isRawDimensionPayload`+ `asStringArray`
  - `services/agents/tensionOptimizer/TensionOptimizerAgent.ts` — 13× `||→??`
    - 5× `async→sync`
  - `services/agents/thematicMining/ThematicMiningAgent.ts` — 11× `||→??`
    - 4× `async→sync`
  - `controllers/workflow.controller.ts` — 12× `||→??`
    - 4× `async→sync`
- معالجة web يدوية:
  - `hooks/useProject.ts` — 17 موقع `no-floating-promises`معالج بـ `await Promise.all([...])`

### النتائج المُقاسة (after vs before)

البندقبلبعدفرقBackend lint errors1,3741,301-73Web lint errors1,4491,104-345Web lint warnings3631-5**إجمالي errors2,8232,405-418 (-14.8%**)Backend test files فاشلة116-5 (-45%)Backend tests فاشلة4645-1Backend tests ناجحة1,0281,041+13

### ما ثبت

- لا انحدار في tests (تحسّن +13 اختبار ناجح)
- لا تخفيف في contract baseline أو eslint config أو tsconfig
- إضافة runtime validation فعلي (zod) في cineai.service.ts بدلاً من `Record<string, any>`→ تشديد لا تخفيف
- نمط `await Promise.all([invalidateQueries...])`في useProject.ts → invalidation فعلي قبل اعتبار mutation ناجحة

### ما بقي مفتوحاً

- 1,301 خطأ backend (معظمه يحتاج narrowing عميق على ملفات خدمات و controllers)
- 1,104 خطأ web (معظمه refactor: `max-lines-per-function`و `complexity`و `max-lines`)
- 6 ملفات backend tests فاشلة (45 اختبار) — تحتاج تشخيص لكل ملف
- جلسة 7 لـ Agentic RAG لم تُنفَّذ بعد

### حالة الفرع

- branch: `main`
- HEAD: `02b240e614eca08b0c904f7e45212532e7e5140b`
- working tree غير نظيفة — تعديلات جلستي 5+6 جاهزة للـ commit
- التقرير الكامل: `output/production-readiness/session5-6-report.md`
...
الفحصالنتيجة`pnpm exec turbo run build --dry-run=json | grep -i tiptap`✅ لا يُرجع شيئًا — القيمة لا تظهر`pnpm exec turbo run build --dry-run=text | grep "Passed Through"`✅ كل قيم `Passed Through Env Vars` فارغةالرسم البياني للمهام✅ سليم — مهام `build` بـ hash جديد، الاعتماديات صحيحة`env` array لمهمة build✅ 15 متغير NEXT_PUBLIC\_\* + SENTRY\_\* محفوظة
...
البناء السابق (`31d409b`) على `main` أصدر **71 تحذيرًا** `could not determine a source map reference` على Vercel. السبب الجذري بعد فحص [Untitled-4.txt](../Untitled-4.txt): 71 ملف client بنطاق `-270` إلى `-410` يخرج إلى القرص بصيغة `(no sourcemap found)`، لأن `config.devtool = 'eval-source-map'` يدويًا في next.config.mjs لا ينتج `.map` منفصلة لكل chunk. الخطة السابقة (`sourcemaps.assets` whitelist وحدها) لم تحل لأنها تحدد _ما يُرفع_، لا تجبر _توليد maps_.
...
#الأمرالنتيجة1`pnpm install --frozen-lockfile --dry-run`❌ pnpm 10.32.1 لا يدعم flag `--dry-run` (`Unknown option`) — الفحص غير قابل للتنفيذ كما طُلب2`pnpm exec tsc --noEmit` على `packages/core-memory` (بديل لغياب tsconfig جذر)✅ نظيف، صفر أخطاء2-alt`pnpm exec turbo run type-check --dry-run=text`✅ الرسم البياني صحيح، 1767 input file لـ web type-check3`pnpm exec turbo run build --dry-run=json`⚠️ نجح بنيويًا لكن **سرّب القيمة الكاملة لـ** `TIPTAP_PRO_TOKEN` في حقل `passthrough` ضمن stdout4`pnpm list --depth=0 --json`✅ 8 deps جذرية، lockfile متّسق
...
5. **🔴 [session-state.md](http://session-state.md) قديم:** يقول الفرع `main` و 27 ملف drift، الواقع الفرع `fix/sentry-sourcemaps-root` و 3 ملفات تغيير محلية فقط، آخر commit مختلف.
...
- تحديث [session-state.md](http://session-state.md) ليطابق الواقع (يحتاج `pnpm agent:bootstrap` أو تحديث يدوي).
...
الطبقةreleasetracePropagationsendDefaultPiiحالةBackend (Express)`the-copy-backend@<sha>`✅ جديد`false`جاهز — ينتظر `SENTRY_AUTH_TOKEN` و `FRONTEND_URL` على RailwayNext.js Server (nodejs)`the-copy-web@<sha>` (جديد)✅ كان موجوداً`false` (جديد)جاهز — ينتظر env vars VercelNext.js Edge`the-copy-web@<sha>` (جديد)n/a`false` (جديد)جاهزNext.js Client (Browser)`the-copy-web@<sha>` (جديد)✅ كان موجوداً`false` (جديد)جاهز
...
الطبقةملف التهيئةحالةBackend (Express)`apps/backend/src/config/sentry.ts`جاهز كود — ينتظر `SENTRY_DSN` على RailwayNext.js Server`apps/web/src/instrumentation.ts` (`nodejs`)جاهز كود — ينتظر `NEXT_PUBLIC_SENTRY_DSN` على VercelNext.js Edge`apps/web/src/instrumentation.ts` (`edge`)جاهز كود — ينتظر `NEXT_PUBLIC_SENTRY_DSN` على VercelNext.js Client (Browser)`apps/web/src/instrumentation-client.ts`**جديد** — ينتظر `NEXT_PUBLIC_SENTRY_DSN` على Vercel
...
المعرّفالسببA-002P2 (lint hygiene). 64 خطأ، أغلبها parsing config — يحتاج إصلاح ESLint config وليس إصلاحًا ملفًا ملفًا. خارج blast-radius جلسة واحدة.A-003P3 — مُرحَّل بحكم الترتيب الحاكم في العقد (P0→P1→P2 فقط، P3 لاحقًا).A-004..A-011P1/P2 لكل اختبار فاشل — 68 اختبارًا تحتاج تحقيق نطاقي مسبق قبل أي إصلاح (test fixtures مكسورة، paths مرفوضة، timing flaky). تنفيذها العشوائي خطر مباشر على الأخضر الموروث (8/8 cinematography).A-009فاشل في cinematography-analysis-fallback — pre-existing flaky timing (5050ms). يحتاج تحليل timing أولًا.O-001..O-005كلها P2/P3 بحجم ضخم (267 `any`، 133 `console`، 18 TODO، 6 ملفات فوق 1000 سطر). إغلاقها يحتاج خطة هيكلية متعددة الجولات.D-001`[UN]` — يلزم تحقيق إضافي (هل غياب listeners على 5433/6379/8080 عطل أم وضع تشغيلي مقصود؟).
...
1. `apps/backend/.env.example` — قائمة صغيرة بالمتغيرات المطلوبة فعلياً للإقلاع (NODE_ENV, PORT, DATABASE_URL, JWT_SECRET, CORS_ORIGIN, RATE_LIMIT\_\*, NEXT_PUBLIC_BACKEND_URL). الجذر `.env.example` يبقى المرجع الكامل.
...
2026-04-29T13:14:16.2266229+03:00
...
- تشغيل أمر التحقق الكامل المطلوب.
- إصلاح أخطاء البناء وفحص الأنواع التي منعت بناء الواجهة.
- إصلاح تجاوز ميزانية الحزمة دون تغيير حدود الحارس أو تعطيله.

### ما الذي تم تغييره

- ضبط عقود الاستجابة الاحتياطية في وكلاء تحليل الدراما.
- إضافة تحقق صريح لتحميل إعدادات الوكلاء الديناميكية.
- تصحيح تحويل المشروع المخزن إلى شكل مشروع صالح.
- تصحيح عقود متجر المشاريع وخدمة صحة الخلفية.
- توحيد مصدر أنواع التتبع داخل ملف التتبع.
- تصحيح شرط منطقي في مطبع ماركداون.
- تخفيف تحميل مساري المخرج للأوامر الحية وخريطة المتسابقين عبر الاعتماد على مسارات فرعية أخف وحاويات ثابتة.
...
pnpm run guard:budgets
pnpm run validate
pnpm --dir apps/web exec next start -p 5012 -H 127.0.0.1
```

### نتيجة التحقق التنفيذي

- نجح بناء الواجهة بعد الإصلاحات.
- نجح حارس الميزانية بعد خفض أسوأ حمل جافاسكربت إلى الحد المسموح.
- نجح أمر التحقق الكامل المطلوب برمز خروج صفر.
- رجع مسار الأوامر الحية من الخادم المحلي برمز 200.
- رجع مسار خريطة المتسابقين من الخادم المحلي برمز 200.

### ملاحظات

- لم يتم تخفيف ملفات الفحص أو الاختبار أو الميزانيات.
- فحص الأنواع العقدي يعرض عدادات مديونية نوعية ضمن آلية العقد، لكنه لم يفشل أمر التحقق الكامل.
...
- 5 أخطاء type-check جديدة على web موروثة من main (مرشَّحة كـ R-_ أو D-_ في triage الجولة 098):
