...
describe('SSEService > Connection Initialization', () => {
  it('should initialize SSE connection with correct headers', () => {
    sseService.initializeConnection('client-1', asResponse(mockResponse), 'user-123');
  ...
  it('should support anonymous connections', () => {
    sseService.initializeConnection('client-anonymous', asResponse(mockResponse));
...
describe('SSEService > Sending Events', () => {
  ...
  it('should send event to specific client', () => {
    ...
    expect(mockResponse.write).toHaveBeenCalledWith(expect.stringContaining('Test message'));
  });
...
describe('SSEService > Room Broadcasting', () => {
  ...
  beforeEach(() => {
    ...
    sseService.initializeConnection('client-1', asResponse(mockResponse), 'user-123');
    sseService.initializeConnection('client-2', asResponse(mockResponse2), 'user-456');
...
describe('SSEService > User Broadcasting', () => {
  ...
  beforeEach(() => {
    ...
    sseService.initializeConnection('client-1', asResponse(mockResponse), 'user-123');
    sseService.initializeConnection('client-2', asResponse(mockResponse2), 'user-123');
...
describe('SSEService > Global Broadcasting', () => {
  ...
  beforeEach(() => {
    ...
    sseService.initializeConnection('client-1', asResponse(mockResponse), 'user-1');
    sseService.initializeConnection('client-2', asResponse(mockResponse2), 'user-2');
...
describe('SSEService > Data Streaming', () => {
  ...
  it('should stream data with custom event type', () => {
    expect(sseService.streamData('client-1', 'Analysis log', 'analysis:log')).toBe(true);
...
describe('SSEService > Service Shutdown', () => {
  ...
  it('should disconnect all clients on shutdown', () => {
    ...
    expect(mockResponse.write).toHaveBeenCalledWith(expect.stringContaining('disconnected'));
    expect(mockResponse.end).toHaveBeenCalled();
