...
export class EnhancedSelfCritiqueModule {
  ...
  async applyEnhancedCritique(request: CritiqueRequest): Promise<EnhancedCritiqueResult> {
    const { output: originalOutput, task, context } = request;
    const config = this.resolveConfig(request);

    logger.info(`[Enhanced Critique] Starting critique for ${config.agentName}`, {
      dimensions: config.dimensions?.length || 0, maxIterations: config.maxIterations || 3
    });

    const dimensionScores = await this.evaluateAllDimensions(originalOutput, task, context, config);
    const overallScore = calculateOverallScore(dimensionScores, config.dimensions || []);
    const overallLevel = determineLevel(overallScore, config.thresholds || {
      excellent: 0.85, good: 0.7, satisfactory: 0.55, needsImprovement: 0.4
    });
    ...
    logger.info(`[Enhanced Critique] Complete for ${config.agentName}`, {
      overallScore, overallLevel, improved: state.improved, iterations: state.iterations
    });

    return {
      originalOutput, refinedOutput: state.refinedOutput,
      improved: state.improved, iterations: state.iterations,
      dimensionScores, overallScore, overallLevel,
      improvementScore: state.improvementScore,
      critiqueNotes, improvementPlan: improvementPlanResult
    };
  }
  ...
  private resolveConfig(request: CritiqueRequest): CritiqueConfiguration {
    ...
    if (!retrieved) throw new Error(`No critique configuration found for ${context.taskType}`);
    return retrieved;
  }
  ...
  private async applyImprovementIfNeeded(
    ...
    const improvementScore = result.improved
      ? await this.calculateImprovementScore(originalOutput, result.refinedOutput)
      : 0;
  ...
  private async evaluateAllDimensions(
    output: string, task: string, context: CritiqueContext, config: CritiqueConfiguration
  ): Promise<DimensionScore[]> {
    const scores: DimensionScore[] = [];
    for (const dimension of config.dimensions) {
      logger.debug(`[Enhanced Critique] Evaluating dimension: ${dimension.name}`);
      scores.push(await this.evaluateDimension(output, task, context, dimension));
    }
    return scores;
  }

  private async evaluateDimension(
    output: string, task: string, context: CritiqueContext, dimension: CritiqueDimension
  ): Promise<DimensionScore> {
    const prompt = buildDimensionEvaluationPrompt(output, task, context.originalText, dimension);
    try {
      const response = await geminiService.generateText(prompt, { temperature: 0.2, maxTokens: 2048 });
      return parseDimensionResponse(response, dimension);
    } catch {
      logger.error(`[Enhanced Critique] Error evaluating ${dimension.name}`);
      return { dimension: dimension.name, score: 0.5, level: "satisfactory", strengths: [], weaknesses: [], suggestions: [] };
    }
  }
  ...
  private async performImprovementCycles(
    ...
    return { refinedOutput: currentOutput, improved, iterations: improved ? config.maxIterations : 0 };
  }
  ...
  private async runSingleCycle(
    ...
    try {
      ...
      if (hasImproved) {
        logger.debug(`[Enhanced Critique] Improvement cycle ${iteration} successful`);
        return improved;
      }
      logger.debug(`[Enhanced Critique] No improvement after cycle ${iteration}`);
      return null;
    } catch {
      logger.error(`[Enhanced Critique] Error in improvement cycle ${iteration}`);
      return null;
    }
  }
...
قدم التحليل المحسّن فقط بدون شروحات إضافية:`;
  ...
  private async verifyImprovement(original: string, improved: string): Promise<boolean> {
  ...
  private async calculateSimilarity(text1: string, text2: string): Promise<number> {
    ...
    try {
      const response = await geminiService.generateText(prompt, { temperature: 0.1, maxTokens: 50 });
  ...
  private async calculateImprovementScore(original: string, refined: string): Promise<number> {
