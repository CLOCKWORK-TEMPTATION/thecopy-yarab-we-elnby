...
export class CausalGraphBuilder {
  ...
  async buildCausalGraph(text: string): Promise<CausalPlotGraph> {
    logger.info("[Causal Graph] Building causal plot graph...");

    // Step 1: استخراج الأحداث
    logger.info("[Causal Graph] Extracting events...");
    const events = await this.extractEvents(text);
    logger.info(`[Causal Graph] Found ${events.length} events`);

    // Step 2: بناء العقد (Nodes)
    logger.info("[Causal Graph] Building nodes...");
    const nodes = await this.buildNodes(events);

    // Step 3: تحديد العلاقات السببية (Edges)
    logger.info("[Causal Graph] Building edges...");
    const edges = await this.buildEdges(nodes, text);
    logger.info(`[Causal Graph] Found ${edges.length} causal relations`);

    // Step 4: بناء الخط الزمني
    const timeline = nodes.map((n) => n.event);

    // Step 5: تحليل السببية
    logger.info("[Causal Graph] Analyzing causality...");
    const causality = await this.analyzeCausality(nodes, edges);

    logger.info("[Causal Graph] Complete");
...
`;
    ...
    try {
      ...
      logger.error("[Causal Graph] Error assessing importance:", error);
      return 0.5;
    }
  }
...
`;
    ...
    try {
      const parsed =
        await geminiService.generateJson<CausalRelationAssessment>(prompt);
      ...
      logger.error("[Causal Graph] Error detecting relation:", error);
      return null;
    }
  }
...
`;
    ...
    try {
      ...
      logger.error("[Causal Graph] Error explaining causality:", error);
