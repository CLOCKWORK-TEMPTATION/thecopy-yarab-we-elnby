...
async function exampleFastAnalysis() {
  ...
  logger.info('Analysis completed in:', result.metrics.totalExecutionTime, 'ms');
  logger.info('Parallelization efficiency:', result.metrics.parallelizationEfficiency);
}
...
async function exampleCompleteAnalysis() {
  ...
  logger.info('- Average confidence:', result.metrics.confidenceDistribution.avg);
  logger.info('- Total execution time:', result.metrics.totalExecutionTime, 'ms');
  logger.info('- Average agent time:', result.metrics.avgAgentExecutionTime, 'ms');
}
...
async function exampleCreativeWorkflow() {
  ...
  logger.info('Generated content quality:', result.metrics.confidenceDistribution.avg);
}
...
async function exampleWithMonitoring() {
  ...
  workflowExecutor.on('step-started', (event) => {
    logger.info(`[${event.timestamp.toISOString()}] Step started: ${event.stepId}`);
  });
  ...
  workflowExecutor.on('step-failed', (event) => {
    logger.error(`[${event.timestamp.toISOString()}] Step failed: ${event.stepId}`);
...
async function exampleAdvancedModules() {
  ...
  logger.info('Confidence distribution:', result.metrics.confidenceDistribution);
}
...
async function exampleErrorHandling() {
  ...
  logger.info('Lenient workflow completed with', lenientResult.metrics.successRate * 100, '% success');
}
...
async function exampleHybridApproach() {
  ...
  logger.info('Quick analysis confidence:', quickResult.metrics.confidenceDistribution.avg);
  ...
  if (quickResult.metrics.confidenceDistribution.avg < 0.7) {
    ...
    logger.info('Complete analysis finished with confidence:', 
