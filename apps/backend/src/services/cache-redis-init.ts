...
import type { CacheRedisConfig, RedisClientInstance, RedisRetryOptions } from './cache.types';
...
function buildRedisConfig(): CacheRedisConfig {
  ...
  if (process.env.REDIS_SENTINEL_ENABLED === 'true') {
    const sentinels = (
      process.env.REDIS_SENTINELS ?? '127.0.0.1:26379,127.0.0.1:26380,127.0.0.1:26381'
    )
      .split(',')
      .map((s) => {
  ...
  config.retry_strategy = (options: RedisRetryOptions): Error | number | undefined => {
 ...
 */
export function buildRedisClient(callbacks: RedisClientCallbacks): RedisClientInstance {
