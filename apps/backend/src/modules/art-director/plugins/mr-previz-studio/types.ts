...
export interface SetupCameraInput {
  sceneId: string;
  camera: Partial<VirtualCamera>;
}

export interface SimulateCameraMovementInput {
  sceneId: string;
  cameraId: string;
  movement: CameraMovement;
}

export interface ConfigureLightingInput {
  sceneId: string;
  lighting: Partial<XRLighting>;
}

export type XRExportFormat = "gltf" | "usdz" | "fbx" | "obj";
export type XRExportTarget = "vr" | "ar" | "mr" | "all";

export interface ExportSceneInput {
  sceneId: string;
  format: XRExportFormat;
  target: XRExportTarget;
}

export interface GenerateArPreviewInput {
  sceneId: string;
  targetDevice: string;
}

export interface GenerateVrWalkthroughInput {
  sceneId: string;
  waypoints?: Vector3D[];
}
