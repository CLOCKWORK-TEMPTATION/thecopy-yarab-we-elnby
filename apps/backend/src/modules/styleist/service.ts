...
class StyleistService {
  ...
  async createDesign(input: CreateDesignInput) {
    const [design] = await db.insert(costumeDesigns).values({
  ...
  async assignSceneCostume(input: AssignSceneCostumeInput) {
    const [assignment] = await db.insert(sceneCostumes).values({
      projectId: input.projectId,
  ...
  async getSceneCostumesByProject(projectId: string) {
    return db.select().from(sceneCostumes)
      .where(eq(sceneCostumes.projectId, projectId))
      .orderBy(desc(sceneCostumes.createdAt));
  }

  async updateSceneCostume(sceneId: string, projectId: string, updates: Partial<AssignSceneCostumeInput>) {
    const [updated] = await db.update(sceneCostumes)
      .set({
