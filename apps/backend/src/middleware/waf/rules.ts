/** OWASP CRS-based Detection Patterns — all regex pattern collections used by the WAF. */
import type { WAFRule } from "./config";
...
const XSS_PATTERNS: WAFRule[] = [
  ...
  {
    id: "941160",
    name: "XSS Filter - Category 6: Obfuscated Alert Payload",
    description: "Detects inline alert payloads commonly used in XSS probing",
    pattern: /(^|['"`\-\s])alert\s*\([^)]*\)(?=['"`\-\s]|$)/gi,
    locations: ["body", "query"],
    action: "block",
    severity: "medium",
    enabled: true,
...
export const WAF_PATTERNS = {
  SQL_INJECTION: SQL_INJECTION_PATTERNS, XSS: XSS_PATTERNS,
  COMMAND_INJECTION: COMMAND_INJECTION_PATTERNS, PATH_TRAVERSAL: PATH_TRAVERSAL_PATTERNS,
  PROTOCOL_ATTACK: PROTOCOL_ATTACK_PATTERNS, BOT_DETECTION: BOT_DETECTION_PATTERNS,
};
