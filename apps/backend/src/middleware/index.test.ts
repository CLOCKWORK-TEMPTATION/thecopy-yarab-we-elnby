...
vi.mock("./log-sanitization.middleware", () => ({
  sanitizeRequestLogs: vi.fn((_req: unknown, _res: unknown, next: () => void) =>
    next(),
  ),
}));

vi.mock("./slo-metrics.middleware", () => ({
  sloMetricsMiddleware: vi.fn(
  ...
  getSLOStatus: vi.fn(() => ({})),
  SLO_TARGETS: {
    api: { availability: 0.999, latencyP95Ms: 500 },
    auth: { successRate: 0.995 },
    gemini: { successRate: 0.95 },
    database: { availability: 0.9995 },
  },
  ERROR_BUDGETS: {
    api: { availability: 43.2 },
    auth: { successRate: 216 },
    gemini: { successRate: 2160 },
    database: { availability: 21.6 },
  },
}));
