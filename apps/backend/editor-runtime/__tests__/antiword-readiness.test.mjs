...
test("antiword-readiness: عند الفشل، يكون reason صريحًا", async (t) => {
  ...
  if (!result.antiwordReady) {
    ...
    ok(
      result.reason.includes("antiword") || result.reason.includes("ANTIWORDHOME"),
...
test("antiword-readiness: المسارات متسقة مع الحالة", async (t) => {
  ...
  if (result.antiwordBinaryAvailable) {
    ok(result.antiwordPath !== null, "antiwordPath يجب أن يكون موجود عند توفر antiword");
    ok(
  ...
  if (result.antiwordHomeExists) {
    ok(result.antiwordHome !== null, "antiwordHome يجب أن يكون موجود عند وجود ANTIWORDHOME");
    ok(
...
test("antiword-readiness: حالات التناسق الكامل", async (t) => {
  ...
  if (
    result.antiwordBinaryAvailable &&
    result.antiwordHomeExists
  ) {
    strictEqual(result.antiwordReady, true, "عند توفر كل شيء، antiwordReady يجب أن يكون true");
    strictEqual(result.docIntakeReady, true, "عند توفر كل شيء، docIntakeReady يجب أن يكون true");
    strictEqual(result.reason, null, "عند توفر كل شيء، reason يجب أن يكون null");
  }

  // الحالة 2: antiword غير متاح
  if (!result.antiwordBinaryAvailable) {
    strictEqual(result.antiwordReady, false, "عند عدم توفر antiword، antiwordReady يجب أن يكون false");
    strictEqual(result.docIntakeReady, false, "عند عدم توفر antiword، docIntakeReady يجب أن يكون false");
  ...
  if (result.antiwordBinaryAvailable && !result.antiwordHomeExists) {
    strictEqual(result.antiwordReady, false, "عند عدم وجود ANTIWORDHOME، antiwordReady يجب أن يكون false");
    strictEqual(result.docIntakeReady, false, "عند عدم وجود ANTIWORDHOME، docIntakeReady يجب أن يكون false");
