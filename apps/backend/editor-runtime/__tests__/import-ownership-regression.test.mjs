...
describe('Non-regression Tests - ملكية الاستيراد', () => {
  ...
  it('no file input in directors-studio ScriptUploadZone', async () => {
    ...
    try {
      ...
      assert.fail(
        `فشل قراءة ScriptUploadZone.tsx: ${error.message}`
      );
    }
  ...
  it('no mammoth import in directors-studio', async () => {
    ...
    try {
      ...
      assert.fail(
        `فشل قراءة fileExtractor.ts: ${error.message}`
      );
    }
  ...
  it('no extractTextFromFile actual implementation', async () => {
    ...
    const hasDisabledCheck = /EXTRACTION_DISABLED|معطّ|disabled|deactivated/i.test(
      content
    );
  ...
  it('ScriptUploadZone shows deactivation message', async () => {
    ...
    const hasArabicDeactivationMessage = /الاستيراد.*معطّ|معطّ.*الاستيراد/i.test(
      content
    );
  ...
  it('NoProjectSection does not import ScriptUploadZone', async () => {
    ...
    try {
      ...
      assert.fail(
        `فشل قراءة NoProjectSection.tsx: ${error.message}`
      );
    }
...
describe('Ownership Verification - تحقق الملكية', () => {
  ...
  it('ScriptUploadZone redirect button points to /editor', async () => {
    ...
    const redirectsToEditor = /'\/editor'|"\/editor"|router\.push\(['"]*\/editor/.test(
      content
    );
...
describe('Script Page Deactivation - تعطيل صفحة تحرير السيناريو', () => {
  it('no Textarea in directors-studio script page', async () => {
    ...
    try {
      ...
      assert.fail(
        `فشل قراءة script/page.tsx: ${error.message}`
      );
    }
  ...
  it('no Download button in directors-studio script page', async () => {
    ...
    const hasDownloadButton = /تحميل|تنزيل|download|onDownload|handleDownload/i.test(content);
    assert.strictEqual(
      hasDownloadButton,
      false,
  ...
  it('no Analyze action in directors-studio script page', async () => {
    ...
    const hasAnalyzeAction = /تحليل|analyze|cineai\/analyze|onAnalyze|handleAnalyze/i.test(content);
    assert.strictEqual(
      hasAnalyzeAction,
      false,
  ...
  it('no POST to /api/cineai/analyze from directors-studio script page', async () => {
    ...
    const hasCineaiEndpoint = /api\/cineai|cineai\/analyze|fetch.*analyze/i.test(content);
    assert.strictEqual(
      hasCineaiEndpoint,
      false,
