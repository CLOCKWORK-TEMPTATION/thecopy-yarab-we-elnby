...
const deriveFirstVisibleSourceKind = (fileType, usedOcr, method) => {
  if (fileType === "paste") {
    return "user-paste";
  }

  if (usedOcr || method === "ocr-mistral") {
    return "ocr";
  }
...
const deriveProgressiveStage = (method) =>
  method === "karank-engine-bridge" ? "karank-visible" : "first-visible";
  ...
  return {
    ...
    mismatchReportPath:
      typeof result.mismatchReportPath === "string"
        ...
        : undefined,
    extractionMeta: {
      progressiveStage: deriveProgressiveStage(method),
      sourceType: fileType,
      firstVisibleSourceKind: deriveFirstVisibleSourceKind(
        fileType,
        usedOcr,
