  ...
  return await new Promise((resolve, reject) => {
    ...
    server.listen(0, '127.0.0.1', () => {
      ...
      if (!address || typeof address === 'string') {
        reject(new Error('Failed to resolve an ephemeral port for the runtime server.'));
        return;
      }
...
async function waitForServer(port: number): Promise<void> {
  ...
  throw new Error(`Runtime server on port ${port} did not become ready in time.`);
}

function extractCookiePair(setCookieHeader: string | null): { cookie: string; token: string } {
  if (!setCookieHeader) {
    throw new Error('Expected a CSRF cookie to be set by the app-state GET request.');
  }
...
describe('budget runtime integration', () => {
  ...
  it(
    'ينشئ الميزانية ويصدر الملف ويحفظ الحالة مع سجل fallback منظم',
    async () => {
      const requestBody = {
      ...
      expect(generationPayload.data.analysis.summary).toContain('مطاردة على الكورنيش');
      expect(generationPayload.data.budget.sections[0]?.categories[0]?.items.length).toBeGreaterThan(0);
  ...
  it(
    'يرجع 503 عندما يكون المسار في وضع provider-only بدون اعتماد صالح',
    async () => {
