...
vi.mock("@/ai/gemini-service", () => ({
  geminiService: {
    generateContent: vi
      .fn()
      .mockResolvedValue("Mock AI response for audience resonance analysis"),
  },
}));
...
describe("AudienceResonanceAgent — Configuration", () => {
...
describe("AudienceResonanceAgent — Success Path", () => {
  let agent: AudienceResonanceAgent;
  ...
  it("should analyze audience segments appropriately", async () => {
    const input: StandardAgentInput = {
      input: "حلل صدى المحتوى عبر شرائح جماهير مختلفة",
      options: {
        enableRAG: true,
      },
      context: {
...
describe("AudienceResonanceAgent — Low Confidence Path", () => {
  let agent: AudienceResonanceAgent;
...
describe("AudienceResonanceAgent — Hallucination Detection", () => {
  let agent: AudienceResonanceAgent;
...
describe("AudienceResonanceAgent — Post-Processing Quality", () => {
  let agent: AudienceResonanceAgent;
...
describe("AudienceResonanceAgent — Advanced Options", () => {
  let agent: AudienceResonanceAgent;
...
describe("AudienceResonanceAgent — Error Handling", () => {
  let agent: AudienceResonanceAgent;
...
describe("AudienceResonanceAgent — Integration Tests", () => {
  let agent: AudienceResonanceAgent;
  ...
  it("should execute full pipeline with all modules enabled", async () => {
    const input: StandardAgentInput = {
      ...
      context: {
        originalText: "عمل درامي اجتماعي يتناول قضايا معاصرة بعمق وحساسية",
        targetAudience: {
          demographics: {
            ageRange: "25-50",
            gender: "متنوع",
    ...
    expect(result.metadata?.constitutionalViolations).toBeGreaterThanOrEqual(0);
