...
function makeAgent() {
  const agent = new TensionOptimizerAgent();
  vi.clearAllMocks();
  return agent;
}

describe("TensionOptimizerAgent — Configuration", () => {
...
describe("TensionOptimizerAgent — Success Path", () => {
  let agent: TensionOptimizerAgent;
...
describe("TensionOptimizerAgent — Low Confidence Path", () => {
  let agent: TensionOptimizerAgent;
...
describe("TensionOptimizerAgent — Hallucination Detection", () => {
  let agent: TensionOptimizerAgent;
  ...
  it("should maintain high confidence for well-supported analysis", async () => {
    const input: StandardAgentInput = {
      input: "حلل التوتر في نص موثق بشكل جيد",
      options: {
        enableRAG: true,
        confidenceThreshold: 0.75,
      },
      context: {
        originalText: "مشهد درامي واضح مع توتر محدد",
...
describe("TensionOptimizerAgent — Post-Processing and Quality", () => {
  let agent: TensionOptimizerAgent;
...
describe("TensionOptimizerAgent — Error Handling", () => {
  let agent: TensionOptimizerAgent;
...
describe("TensionOptimizerAgent — Advanced Options", () => {
  let agent: TensionOptimizerAgent;
...
describe("TensionOptimizerAgent — Scene Breakdown Analysis", () => {
  let agent: TensionOptimizerAgent;
...
describe("TensionOptimizerAgent — Integration with Standard Pattern", () => {
  let agent: TensionOptimizerAgent;
  ...
  it("should execute full standard pattern pipeline", async () => {
    const input: StandardAgentInput = {
      input: "حلل وحسّن التوتر مع كل الخيارات المتقدمة",
      options: {
        enableRAG: true,
        enableSelfCritique: true,
        enableConstitutional: true,
