...
import { supportsMediaCapture } from "../../../lib/self-tape";
  ...
  return (
    <div className="grid grid-cols-1 gap-6 lg:grid-cols-2">
          ...
          <CardContent className="space-y-6">
            {!mediaCaptureSupported && (
              <Alert className="border-yellow-500/40 bg-yellow-500/10 text-yellow-200">
                <AlertDescription>
                  هذه البيئة لا تدعم التسجيل الفعلي. ما يزال بإمكانك
                  استعراض الواجهة، لكن التسجيل سيتطلب متصفحاً حديثاً.
                </AlertDescription>
              </Alert>
            )}
              ...
              {isRecording && (
                <>
                  <div className="absolute right-4 top-4 flex items-center gap-2 rounded-full bg-black/60 px-3 py-2">
                    ...
                    <span className="font-mono text-red-300">{formatTime(recordingTime)}</span>
                  </div>
            ...
            <div className="grid grid-cols-1 gap-3 md:grid-cols-3">
              ...
              {!isRecording ? (
                <Button
                  ...
                  disabled={cameraState === "requesting" || isFinalizingTake || !mediaCaptureSupported}
                >
                  🎬 بدء التسجيل
                </Button>
              ) : (
                <Button className="bg-black/22 text-white hover:bg-white/8" onClick={stopRecording}>
                  ⏹️ إنهاء التسجيل
                </Button>
              )}
            ...
            <div className="grid grid-cols-3 gap-4 border-t border-purple-500/30 pt-4">
              <div className="text-center">
                <p className="text-3xl font-bold text-purple-400">{availableTakes.length}</p>
              ...
              <div className="text-center">
                <p className="text-3xl font-bold text-blue-400">{formatTime(totalDuration)}</p>
      ...
      <CardSpotlight className="overflow-hidden rounded-[22px] bg-black/18 border-purple-500/30">
        <Card className="bg-black/18 border-purple-500/30 bg-transparent">
          <CardHeader>
            <CardTitle className="text-white">📋 التسجيلات ({availableTakes.length})</CardTitle>
            <CardDescription className="text-purple-300">
              التسجيلات الحية تبقى قابلة للتصدير داخل الجلسة الحالية،
              بينما تُحفظ البيانات الوصفية محلياً بين الزيارات.
            </CardDescription>
          </CardHeader>
          <CardContent>
                    ...
                    <div>
                      <h4 className="flex items-center gap-2 font-medium text-white">
                        ...
                        {take.source === "captured" ? (
                          <Badge className="bg-emerald-500/20 text-emerald-300">جلسة حيّة</Badge>
                        ) : (
                          <Badge className="bg-black/22/60 text-white/85">نموذج</Badge>
                        )}
                        {take.status === "exported" && (
                          <Badge className="bg-blue-500/20 text-blue-300">مُصدَّر</Badge>
                        )}
                      </h4>
