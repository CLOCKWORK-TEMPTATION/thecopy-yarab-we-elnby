...
interface ComparisonPageProps {
  ...
  setComparisonView: (view: ComparisonView | ((prev: ComparisonView) => ComparisonView)) => void;
  leftTake: Take | undefined;
  rightTake: Take | undefined;
    ...
    <div className="space-y-6">
        ...
        <Card className="bg-black/18 border-purple-500/30 bg-transparent">
          ...
          <CardContent>
            <div className="grid grid-cols-1 items-end gap-4 md:grid-cols-3">
              <div>
                <Label className="mb-2 block text-purple-300">التسجيل الأيسر</Label>
                <Select
              ...
              <div>
                <Label className="mb-2 block text-purple-300">التسجيل الأيمن</Label>
                <Select
      ...
      <div className="grid grid-cols-1 gap-6 md:grid-cols-2">
        {[leftTake, rightTake].map((take, index) => (
          <Card key={take?.id ?? `empty-${index}`} className="bg-black/18 border-purple-500/30">
            <CardHeader className="pb-3">
              <CardTitle className="text-lg text-white">{take?.name ?? "اختر تسجيلاً"}</CardTitle>
              {take && (
                <CardDescription className="text-purple-300">{formatRecordedAt(take.recordedAt)}</CardDescription>
              )}
            </CardHeader>
            <CardContent className="space-y-4">
              {take?.videoUrl ? (
                ...
                >
                  <track kind="captions" src={EMPTY_CAPTIONS_TRACK} srcLang="ar" label="لا توجد ترجمة" />
                </video>
              ) : (
                <div className="flex aspect-video items-center justify-center rounded-[22px] border border-dashed border-purple-500/20 bg-black/40 text-center text-white/55">
                  <div>
                    ...
                    <p className="text-sm">ستبقى المقارنة الرقمية متاحة من خلال التقييمات والملاحظات.</p>
                  </div>
                </div>
              )}
              ...
              {take && (
                <div className="space-y-2 text-sm">
                  <div className="flex justify-between">
                    ...
                    <span className={`font-bold ${getScoreColor(take.score)}`}>{take.score ?? 0}%</span>
                  </div>
                  <div className="flex justify-between">
                  ...
                  <div className="flex justify-between">
                    ...
                    <span className="text-purple-300">{take.notes.length} ملاحظة</span>
                  </div>
                </div>
              )}
            </CardContent>
            ...
            <CardContent>
              <div className="grid grid-cols-1 gap-6 md:grid-cols-3">
                ...
                <div className="rounded-xl bg-black/14 p-4 text-center">
                  ...
                  <p className="text-3xl font-bold text-purple-400">{Math.abs(rightTake.duration - leftTake.duration)}ث</p>
                </div>
                <div className="rounded-xl bg-black/14 p-4 text-center">
                  <p className="mb-2 text-sm text-white/55">النسخة الأفضل</p>
                  <p className="text-xl font-bold text-yellow-400">
                    {(rightTake.score ?? 0) > (leftTake.score ?? 0) ? rightTake.name : leftTake.name}
                  </p>
                </div>
              </div>
            </CardContent>
