...
interface TeleprompterPageProps {
  ...
  setTeleprompterSettings: (settings: TeleprompterSettings | ((prev: TeleprompterSettings) => TeleprompterSettings)) => void;
...
export const TeleprompterPage: React.FC<TeleprompterPageProps> = ({
  scriptText,
      ...
      <CardSpotlight className="overflow-hidden rounded-[22px] bg-black/18 border-purple-500/30">
        <Card className="bg-black/18 border-purple-500/30 bg-transparent">
          ...
          <CardContent className="space-y-6">
            ...
            <div className="space-y-2">
              <Label className="text-purple-300">العد التنازلي: {teleprompterSettings.countdownSeconds} ثوان</Label>
              <Slider
            ...
            >
              {countdown > 0 && (
                <div className="absolute inset-0 z-10 flex items-center justify-center bg-black/80">
                  <span className="text-9xl font-bold text-purple-500">{countdown}</span>
                </div>
              )}
              ...
              >
                {scriptText.split("\n").map((line, index, source) => {
                  const lineProgress = (index / Math.max(source.length - 1, 1)) * 100;
                  const isCurrentLine =
                    teleprompterSettings.highlightCurrentLine &&
