...
export class ImmersiveConceptArt implements Plugin {
  ...
  private async createModel3D(data: {
    ...
    const baseVertexCount = meshVertices[baseMesh];
    ...
    const model: Model3D = {
      ...
      type: data.type,
      geometry: {
        vertices: baseVertexCount,
        faces: Math.round(baseVertexCount * 1.5),
        format: "triangles",
        dimensions,
      },
      materials: [
      ...
      textures: [],
      lod: [
        { level: 0, vertices: baseVertexCount, distance: 0 },
        {
          level: 1,
          vertices: Math.round(baseVertexCount * 0.5),
          distance: 10,
        },
        {
          level: 2,
          vertices: Math.round(baseVertexCount * 0.25),
          distance: 25,
        },
      ],
      interactable: true,
  ...
  private async createEnvironment(data: {
    ...
    const presets: Record<
      NonNullable<typeof data.preset>,
      Pick<Environment3D, "lighting" | "atmosphere">
    > = {
      daylight: {
        lighting: {
    ...
    const presetKey = data.preset ?? "daylight";
  ...
  private async createCharacter(data: {
    ...
    const rigBones: Record<typeof rigType, number> = {
      biped: 65,
      quadruped: 80,
      custom: 40,
    };
  ...
  private generateColorPalette(theme: string): string[] {
    ...
    return palettes[theme.toLowerCase()] ?? defaultPalette;
  }
  ...
  private suggestRelatedThemes(theme: string): string[] {
    ...
    return themeRelations[theme.toLowerCase()] ?? defaultThemes;
  }
