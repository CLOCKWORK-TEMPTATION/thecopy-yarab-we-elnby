...
const readNumericValue = (values: ArrayLike<number>, index: number): number =>
  values[index] ?? 0;
...
const computeEmissionScores = (f: SequenceFeatures): Float64Array => {
  ...
  // ─── basmala
  {
    ...
    if (f.wordCount >= 3 && f.wordCount <= 6 && !f.endsWithColon) {
      adjustNumericValue(scores, idx, 0.5);
    } else {
      adjustNumericValue(scores, idx, -5.0);
    }
  }
  ...
  // ─── scene_header_1 (رقم المشهد — كلمة أو كلمتين)
  {
    ...
    if (f.wordCount <= 3 && !f.endsWithColon && !f.hasActionIndicators) {
      ...
      adjustNumericValue(scores, idx, -4.0);
    }
  }
  ...
  // ─── scene_header_2 (زمن/موقع — قصير بدون نقطتين)
  {
    ...
    if (f.wordCount <= 5 && !f.endsWithColon) {
      ...
      adjustNumericValue(scores, idx, -3.0);
    }
    if (f.hasActionIndicators) adjustNumericValue(scores, idx, -2.0);
  }
  ...
  // ─── action
  {
    ...
    if (f.hasActionIndicators) adjustNumericValue(scores, idx, 3.0);
    if (f.startsWithDash) adjustNumericValue(scores, idx, 2.0);
    if (f.startsWithBullet) adjustNumericValue(scores, idx, 2.0);
    if (f.wordCount >= 5) adjustNumericValue(scores, idx, 1.0);
    if (f.endsWithColon && f.wordCount <= 4)
      adjustNumericValue(scores, idx, -2.0);
    if (f.isParenthetical) adjustNumericValue(scores, idx, -3.0);
  }
  ...
  // ─── character
  {
    ...
    if (f.endsWithColon) adjustNumericValue(scores, idx, 3.0);
    else adjustNumericValue(scores, idx, -6.0);

    if (f.wordCount <= 3) adjustNumericValue(scores, idx, 2.0);
    else if (f.wordCount <= 4) adjustNumericValue(scores, idx, 1.0);
    else adjustNumericValue(scores, idx, -2.0);

    if (f.wordCount > 5) adjustNumericValue(scores, idx, -3.0);

    // مؤشرات وصف = ضد character
    if (f.hasActionIndicators) adjustNumericValue(scores, idx, -3.0);

    // التكرار: أسماء الشخصيات بتتكرر
    if (f.nameRepetitionCount >= 2) adjustNumericValue(scores, idx, 2.5);
    else if (f.nameRepetitionCount === 1) adjustNumericValue(scores, idx, -1.0);

    // البذر: مؤكد من المسح الأولي
    if (f.isPreSeeded) adjustNumericValue(scores, idx, 2.5);

    // ADAPTIVE: الترقيم
    // لو الترقيم الجُملي موجود قبل الـ colon → عقوبة (اسم الشخصية مش جملة)
    // لو مش موجود → 0 (محايد — الكاتب ممكن ميستخدمش ترقيم)
    if (f.hasSentencePunctuation && f.endsWithColon) {
      adjustNumericValue(scores, idx, -2.5);
    }

    // أقواس = مش character
    if (f.isParenthetical) adjustNumericValue(scores, idx, -4.0);
  }
  ...
  // ─── dialogue
  {
    ...
    if (!f.hasActionIndicators) adjustNumericValue(scores, idx, 0.5);

    // ينتهي بنقطتين + قصير = أرجح character مش dialogue
    if (f.endsWithColon && f.wordCount <= 3)
      adjustNumericValue(scores, idx, -2.0);

    // حوار طويل = أرجح dialogue
    if (f.wordCount >= 4 && !f.hasActionIndicators)
      adjustNumericValue(scores, idx, 1.0);

    // ترقيم جُملي = دليل على جملة = dialogue أرجح
    if (f.hasSentencePunctuation) adjustNumericValue(scores, idx, 0.5);

    if (f.startsWithDash) adjustNumericValue(scores, idx, -1.5);
    if (f.startsWithBullet) adjustNumericValue(scores, idx, -1.5);
    if (f.isParenthetical) adjustNumericValue(scores, idx, -2.0);
  }

  // ─── parenthetical
  {
    const idx = TYPE_INDEX.get("parenthetical")!;
    if (f.isParenthetical) adjustNumericValue(scores, idx, 5.0);
    else adjustNumericValue(scores, idx, -5.0);
  }
  ...
  // ─── transition
  {
    ...
    if (f.wordCount <= 5 && !f.endsWithColon && !f.hasActionIndicators) {
      adjustNumericValue(scores, idx, 0.0);
    } else {
      adjustNumericValue(scores, idx, -3.0);
    }
    if (f.wordCount > 6) adjustNumericValue(scores, idx, -3.0);
  }
...
const addForwardPassBias = (
  ...
  adjustNumericValue(emissionScores, typeIdx, bias);
};
...
const viterbiDecode = (
  ...
  for (let s = 0; s < k; s++) {
    prob[s] = readNumericValue(firstEmissions, s);
  }
  ...
  for (let t = 1; t < n; t++) {
    ...
    const currentEmissions = getRequiredItem(allEmissions, t, "emissions");
    ...
    for (let s = 0; s < k; s++) {
      ...
      for (let r = 0; r < k; r++) {
        const score =
          readNumericValue(prob, prevOffset + r) +
          readNumericValue(TRANSITION_MATRIX, r * k + s) +
          readNumericValue(currentEmissions, s);
        if (score > bestScore) {
          bestScore = score;
          bestPrev = r;
        }
  ...
  for (let s = 0; s < k; s++) {
    const candidateScore = readNumericValue(prob, lastOffset + s);
  ...
  for (let t = n - 2; t >= 0; t--) {
    const nextPathIndex = readNumericValue(path, t + 1);
    path[t] = readNumericValue(prev, (t + 1) * k + nextPathIndex);
  }

  // تحويل لـ ViterbiResult[]
  return path.map((typeIdx, lineIdx) => ({
    type: getRequiredItem(VITERBI_TYPES, typeIdx, "viterbi type"),
    score: readNumericValue(prob, lineIdx * k + typeIdx),
  }));
};
...
const detectDisagreements = (
  ...
  for (let i = 0; i < drafts.length; i++) {
    const draft = getRequiredItem(drafts, i, "draft");
    const viterbiResult = getRequiredItem(viterbiResults, i, "viterbi result");
    ...
    const emissions = getRequiredItem(allEmissions, i, "emissions");

    // الفرق بين emission scores للنوعين
    const emissionDiff =
      readNumericValue(emissions, viterbiTypeIdx) -
      readNumericValue(emissions, forwardTypeIdx);
...
export const applyViterbiOverrides = (
  ...
  for (const d of sorted) {
    ...
    const draft = getRequiredItem(classified, d.lineIndex, "classified draft");
    ...
    const nextType =
      d.lineIndex + 1 < classified.length
        ? getRequiredItem(classified, d.lineIndex + 1, "next classified draft")
            .type
        : null;
