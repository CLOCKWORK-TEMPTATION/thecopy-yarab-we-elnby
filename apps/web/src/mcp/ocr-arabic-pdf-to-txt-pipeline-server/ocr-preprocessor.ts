...
export class OCRPreprocessor {
  ...
  private fixCatastrophicStart(text: string, issues: string[]): string {
    ...
    const initialSceneLine = lines[sceneLineIndex];
    if (initialSceneLine === undefined) {
      return text;
    }

    let sceneLine = initialSceneLine.trim();
    ...
    if (/^[٠١٢٣٤٥٦٧٨٩0-9]+$/u.test(sceneLine)) {
      ...
      if (nextNonEmptyIndex >= 0) {
        ...
        const nextSceneLine = lines[sceneLineIndex];
        if (nextSceneLine === undefined) {
          return text;
        }
        sceneLine = nextSceneLine.trim();
      }
    }
    ...
    if (
      ...
      firstLine !== undefined &&
      /^[٠١٢٣٤٥٦٧٨٩0-9]+$/u.test(firstLine.trim())
    ) {
