...
const StationsPipeline = () => {
  ...
  const handleStartAnalysis = () => {
    ...
    startTransition(async () => {
      try {
        // تنفيذ خط الأنابيب — يتضمن fallback تلقائي عند فشل الخادم
        ...
        // حفظ النتائج لصفحة التطوير عبر localStorage + sessionStorage
        ...
        // المفتاح الرئيسي لصفحة التطوير (localStorage للبقاء بين الصفحات)
        try {
          localStorage.setItem(
            "sevenStationsAnalysis",
            JSON.stringify(analysisData)
          );
        } catch {
          // التخزين المحلي قد لا يتوفر — لا يمنع التدفق
        }

        // مفاتيح sessionStorage للتوافق الخلفي
        try {
          ...
          // التخزين قد لا يتوفر — لا يمنع التدفق
        }
        ...
        // إعلام المستخدم بنتيجة التحليل
        const completedCount = nextStatuses.filter(
          (s) => s === "completed"
        ).length;
        ...
        toast({
          ...
          description:
            pipelineResult.mode === "fallback"
              ? `تم التحليل عبر المسار الاحتياطي المحلي (${completedCount}/7 محطات). يمكنك الانتقال لصفحة التطوير.`
              : `تم تحليل النص بنجاح (${completedCount}/7 محطات). يمكنك الانتقال لصفحة التطوير.`,
        });
        ...
        setErrorMessage(
          `فشل التحليل: ${errorMsg}. تحقق من اتصال الشبكة أو حاول مرة أخرى.`
        );
        toast({
          title: "فشل التحليل",
          description: `${errorMsg} — تحقق من اتصال الشبكة أو حاول مرة أخرى.`,
          variant: "destructive",
        });
      }
    });
