    ...
    async () => {
      ...
      const body = (await response.json()) as {
        ...
        data?: {
          projectId: string;
          title: string;
          parsed: { scenes: Array<{ header: string; content: string }> };
        };
        error?: string;
      };
...
describe("اختبارات التكامل: مسار التحليل /api/breakdown/projects/:id/analyze", () => {
  ...
  it(
    ...
    async () => {
      ...
      if (analysisSource) {
        expect(["ai", "fallback"]).toContain(analysisSource);
      }
    },
    config.e2eTimeoutMs
  );
...
describe("اختبارات التكامل: التحقق من ConfigManager الموحَّد", () => {
  it("يُنشئ ConfigManager بالإعدادات الصحيحة من متغيرات البيئة وعلى المنفذ الرسمي", () => {
    ...
    if (!process.env["EDITOR_REAL_TEST_BASE_URL"]) {
      expect(testConfig.baseUrl).toContain(":5000");
    }
